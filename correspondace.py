...
for target in targets.iter_rows(named=True):
    ...
    candidates = candidates[:14] if target["name"] in ["Kathryn Ruemmler", "Lawrence Krauss", "Ariane de Rothschild", "Noam and Valeria Chomsky"] else candidates[:13] if target["name"] in ["Reid Hoffman", "Elon Musk"] else candidates[:12]
