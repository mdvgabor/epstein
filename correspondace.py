...
for target in targets.iter_rows(named=True):
    ...
    candidates = candidates[:22] if target["name"] == "Lawrence Krauss" else candidates[:16] if target["name"] == "Reid Hoffman" else candidates[:17] if target["name"] == "Ariane de Rothschild" else candidates[:14] if target["name"] in ["Kathryn Ruemmler", "Noam and Valeria Chomsky", "Bill Gates"] else candidates[:13] if target["name"] == "Elon Musk" else candidates[:12]
    ...
    for width in range(1, min(len(candidates), 12 if target["name"] == "Ariane de Rothschild" else 11 if target["name"] == "Lawrence Krauss" else 9) + 1):
