...
for target in targets.iter_rows(named=True):
    ...
    candidates = candidates[:17] if target["name"] == "Lawrence Krauss" else candidates[:14] if target["name"] in ["Kathryn Ruemmler", "Ariane de Rothschild", "Noam and Valeria Chomsky", "Bill Gates", "Reid Hoffman"] else candidates[:13] if target["name"] == "Elon Musk" else candidates[:12]
