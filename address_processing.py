...
def clean_body(expr):
    return (
        ...
        .str.replace_all(
            r"(?is)\b(?:please advise|thanks|thank you|ok|okay|yes|no|sure|great|perfect|noted|received)?[,.! ]*\s*sent from my (?:iphone|ipad|android|blackberry|mobile|phone)\b.*$",
            " ",
        )
        .str.replace_all(
            r"(?is)\b(?:please forgive typos|forgive typos|typos?)\.?\s*sent from (?:my )?(?:android|iphone|ipad|blackberry|mobile|phone)\b.*$",
            " ",
        )
        .str.replace_all(
            r"(?is)\bsent (?:using|via) (?:my |the )?(?:verizon wireless )?blackberry(?:®)?(?:\s+\d+)?(?:\s+(?:wireless )?device)?(?:\s+from [^.\n]{1,80})?.*$",
            " ",
        )
        .str.replace_all(
            r"(?is)\bsent from my (?:verizon wireless )?blackberry(?:®)?(?:\s+\d+)?(?:\s+(?:wireless )?device)?(?:\s+from [^.\n]{1,80})?.*$",
            " ",
        )
        .str.replace_all(
            r"(?is)\bsent from mailbox for iphone\b.*$",
            " ",
        )
        .str.replace_all(
            r"(?is)\bsent from yahoo mail for iphone[a-z]*\b.*$",
            " ",
        )
        .str.replace_all(
            r"(?is)\bplease forgive typos\.?\s*sent from android nexus \d+ phone\b.*$",
            " ",
        )
        .str.replace_all(
            r"(?is)\bthis e-?mail was generated by fltplan\.com\b.*$",
            " ",
        )
        .str.replace_all(
            r"(?is)\bpassenger flight briefing\b.*$",
            " ",
        )
        .str.replace_all(
            r"(?is)\bfor printable copy\b.*?\b(?:welcome aboard|departure fbo:|schedule:|arrival at)\b.*$",
            " ",
        )
        .str.replace_all(
