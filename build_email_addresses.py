...
def canonical_email_parts(local_part, domain):
    domain = domain.lower()
    if domain == "googlemail.com":
        domain = "gmail.com"
    return local_part.replace(".", ""), domain
...
def fuzzy_repair_emails(parsed_addresses):
    known = (
    ...
    for row in known.iter_rows(named=True):
        canonical_local, canonical_domain = canonical_email_parts(row["local_part"], row["domain"])
        candidates_by_domain.setdefault(canonical_domain, []).append((canonical_local, row["email"], row["len"]))

    repaired_rows = []
    suspicious = parsed_addresses.filter(
        pl.col("email").is_null()
        & pl.col("normalized_value").is_not_null()
        & pl.col("normalized_value").str.to_lowercase().str.contains(r"§|#|=|grnail|gmall|smail|tmail|corn|cord|\.con\b")
    )
    ...
    repaired = pl.DataFrame(repaired_rows).unique(subset=["id", "address_role", "address_index"], keep="first")
    return (
        parsed_addresses.join(repaired, on=["id", "address_role", "address_index"], how="left")
...
base = (
    ...
    .select("id", "sender", "to_recipients", "cc_recipients", "bcc_recipients", "account_email", "is_promotional")
    ...
    .with_columns(
        ...
        pl.col("to_recipients").fill_null("[]").str.json_decode(pl.List(pl.String)).list.eval(normalize_text(pl.element())).alias("to_recipients_normalized"),
        pl.col("cc_recipients").fill_null("[]").str.json_decode(pl.List(pl.String)).list.eval(normalize_text(pl.element())).alias("cc_recipients_normalized"),
        pl.col("bcc_recipients").fill_null("[]").str.json_decode(pl.List(pl.String)).list.eval(normalize_text(pl.element())).alias("bcc_recipients_normalized"),
...
scalar_addresses = pl.concat(
    [
        base.select("id", pl.lit("sender").alias("address_role"), pl.lit(None, dtype=pl.Int64).alias("address_index"), pl.col("sender").alias("raw_value"), pl.col("sender_normalized").alias("normalized_value")),
        base.select("id", pl.lit("account_email").alias("address_role"), pl.lit(None, dtype=pl.Int64).alias("address_index"), pl.col("account_email").alias("raw_value"), pl.col("account_email_normalized").alias("normalized_value")),
    ],
...
recipient_addresses = pl.concat(
    [
        base.with_columns(pl.int_ranges(0, pl.col("to_recipients_normalized").list.len()).alias("address_index"))
        .select("id", pl.lit("to_recipients").alias("address_role"), "address_index", pl.col("to_recipients_items").alias("raw_value"), pl.col("to_recipients_normalized").alias("normalized_value"))
        .explode("address_index", "raw_value", "normalized_value"),
        base.with_columns(pl.int_ranges(0, pl.col("cc_recipients_normalized").list.len()).alias("address_index"))
        .select("id", pl.lit("cc_recipients").alias("address_role"), "address_index", pl.col("cc_recipients_items").alias("raw_value"), pl.col("cc_recipients_normalized").alias("normalized_value"))
        .explode("address_index", "raw_value", "normalized_value"),
        base.with_columns(pl.int_ranges(0, pl.col("bcc_recipients_normalized").list.len()).alias("address_index"))
        .select("id", pl.lit("bcc_recipients").alias("address_role"), "address_index", pl.col("bcc_recipients_items").alias("raw_value"), pl.col("bcc_recipients_normalized").alias("normalized_value"))
        .explode("address_index", "raw_value", "normalized_value"),
    ],
    how="diagonal_relaxed",
)
...
parsed_addresses = fuzzy_repair_emails(parsed.collect()).with_columns(
