...
recipient_roles = ["to_recipients", "cc_recipients", "bcc_recipients"]


def name_key_exprs(expr):
    normalized = normalize_person_name_expr(
...
def name_key_match_types():
    return pl.DataFrame(
        [
            ("name_key_full", "display_name_full", 0),
            ("name_key_compact", "display_name_compact", 1),
            ("name_key_first_last_initial", "display_name_first_last_initial", 2),
            ("name_key_initial_last", "display_name_initial_last", 3),
            ("name_key_initial_last_prefix", "display_name_initial_last_prefix", 4),
        ],
        schema=["name_key_type", "match_source", "match_priority"],
        orient="row",
    ).with_columns(pl.col("match_priority").cast(pl.UInt8))


def name_key_rows(frame, index, name_column):
    return (
        frame.with_columns(*name_key_exprs(pl.col(name_column)))
        .unpivot(
            index=index,
            on=[
                "name_key_full",
        ...
        .join(name_key_match_types(), on="name_key_type", how="inner")
        .drop("name_key_type")
    )


def unique_person_name_matches(people_frame):
    matches = name_key_rows(people_frame, ["person_id", "person_name"], "person_name")
    return (
        matches.join(
            matches.group_by("match_source", "person_name_key")
...
def add_display_name_matches(participants, name_matches):
    indexed = participants.with_row_index("participant_row")
    best_matches = (
        name_key_rows(indexed, ["participant_row"], "display_name")
    ...
    return indexed.join(best_matches, on="participant_row", how="left").drop("participant_row")
...
def drop_conflicting_email_aliases(frame):
    conflicts = (
        frame.group_by("email")
        .agg(pl.col("person_id").n_unique().alias("person_count"))
        .filter(pl.col("person_count") > 1)
    )
    if conflicts.height == 0:
        return frame
    return frame.join(conflicts.select("email"), on="email", how="anti")
...
def resolve_people(participants, aliases):
    return (
        ...
        .with_columns(
            ...
            pl.coalesce("match_source_alias", "match_source_name").alias("match_source"),
        )
        .drop(
            "person_id_alias",
            "person_name_alias",
    ...
    candidates = (
        resolved_people.filter(
            ...
            & pl.col("match_source").is_in(
                ["display_name_full", "display_name_compact", "display_name_first_last_initial"]
            )
...
jmail_person_ids = people_source.filter(
    pl.col("url").fill_null("").str.contains("jmail.world", literal=True)
).select(pl.col("slug").alias("person_id"))
...
people_aliases = (
    people_source.select(
        pl.col("slug").alias("person_id"),
        pl.col("aliases").fill_null("[]").str.json_decode(pl.List(pl.String)).alias("person_name"),
    )
    .explode("person_name")
    .filter(pl.col("person_name").is_not_null())
)
...
people_csv_emails = drop_conflicting_email_aliases(
    people_source.select(
        pl.col("slug").alias("person_id"),
...
email_scan = pl.scan_parquet(input_dir / "emails.parquet").filter(
    pl.col("is_promotional").fill_null(False).not_()
)
if sample_rows > 0:
    email_scan = email_scan.sort(pl.col("id").hash(seed=1)).head(sample_rows)
    print(f"sample mode: reading {sample_rows:,} random non-promotional emails", flush=True)

processed = (
    email_scan.with_columns(
        pl.col("sender").alias("sender_raw"),
        pl.col("account_email").alias("account_email_raw"),
        *[pl.col(role).alias(f"{role}_raw") for role in recipient_roles],
        ...
        *[decoded_list_expr(role).alias(f"{role}_items") for role in recipient_roles],
...
participant_rows = fuzzy_repair_emails(build_address_table(processed)).with_columns(
    pl.col("id").alias("email_id"),
    pl.col("address_role")
    .replace({"to_recipients": "to", "cc_recipients": "cc", "bcc_recipients": "bcc"})
...
for _ in range(3):
    ...
    if new_aliases.height == 0:
        break
    alias_map = drop_conflicting_email_aliases(
        pl.concat([alias_map, new_aliases], how="diagonal_relaxed").unique(
            subset=["person_id", "email"], keep="first"
        )
    )
    resolved_people = resolve_people(participant_name_matches, alias_map)

unresolved_human_participants = resolved_people.filter(
...
discovered_people = (
    unresolved_human_participants.group_by("email")
    .agg(
        pl.col("email_id").n_unique().alias("email_count"),
...
if discovered_people.height > 0:
    people = pl.concat(
        [people, discovered_people.select("person_id", "person_name", "description")],
        how="diagonal_relaxed",
    ...
    resolved_people = pl.concat(
        [
            ...
            .join(
                discovered_people.select(
                    pl.col("person_id").alias("person_id_discovered"),
                    pl.col("person_name").alias("person_name_discovered"),
                    "email",
                ).with_columns(pl.lit("discovered_email").alias("match_source_discovered")),
                on="email",
                how="left",
            )
            .with_columns(
            ...
            .drop("person_id_discovered", "person_name_discovered", "match_source_discovered"),
        ],
        how="diagonal_relaxed",
    )
